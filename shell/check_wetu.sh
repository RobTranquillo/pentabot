...
# check - https: pentamedia.c3d2.de
PENTAMEDIA=$(/usr/bin/curl --insecure -m3 -I https://pentamedia.c3d2.de/ 2>&1 | grep 'HTTP/' | grep -c "OK")
if [ "$PENTAMEDIA" = "1" ]; then
   echo "Pentamedia=OK" >> "$TMPFILE"
else
   echo "Pentamedia=FAILED" >> "$TMPFILE"
fi
